"""
This is a patched version of ElementTree which preserves the order of attributes
on XML serialisation. In other words, any tree will be serialised with the same
order of attributes as it was deserialised with. Any attributes which did not
appear during deserialisation will appear at the end of the list in the
...
class Element(object):
    ...
    def __init__(self, tag, attrib={},
                 attrib_order=defaultdict(lambda: sys.maxsize, {}),
                 **extra):
        attrib = attrib.copy()
        attrib.update(extra)
        ...
        self.tag = tag
        self.attrib = attrib
        self.attrib_order = attrib_order
        self._children = []
        ...
        if tag is None:
            ...
            if items or namespaces:
                ...
                for k, v in sorted(items,
                                   key=lambda a: elem.attrib_order[a[0]]):
...
class TreeBuilder(object):
    ...
    ##
    # Opens a new element, storing the information about the order of attributes
    # into its property.
    #
    # @param tag The element name.
    # @param attrs A dictionary containing element attributes.
    # @param attrs_order A dictionary mapping attribute names to their relative
    # position in the list.
    # @return The opened element.
    # @defreturn Element

    def start_list(self, tag, attrs, attrs_order):
        self._flush()
        self._last = elem = self._factory(tag, attrs, attrs_order)
        if self._elem:
            self._elem[-1].append(elem)
        self._elem.append(elem)
        self._tail = 0
    ...
    def _start_list(self, tag, attrib_in):
        ...
        if attrib_in:
            for i in range(0, len(attrib_in), 2):
                name = fixname(attrib_in[i])
                attrib[name] = fixtext(attrib_in[i+1])
                attrib_order[name] = i/2
        return self.target.start_list(tag, attrib, attrib_order)
